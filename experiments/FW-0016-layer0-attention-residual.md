...
- Fixture commit: `34fd795bd501f9e6740cab71c0fc12ea097c3be3`
- Candidate commit: `b1d325e77b81914bb329f74a7a152d2b9d28caa8`
- Raw evidence SHA-256:
  `b5f0546c5c06de974186632b8a985717df655b36fe875674dfe04d76bd54a127`
...
The native verifier loads and authenticates all 13 real BF16 tensors once,
then retains one explicit convolution/recurrent state across both decode calls.
It exactly matched seven BF16 captures per step: hyper input, mixed attention
input, injection weights, attention output, convolution state, injection
products, and the composed 10,240-wide residual. It also exactly matched the
F32 recurrent-state capture after each step. The final report therefore records
14 exact BF16 and two exact F32 capture hashes across two steps and 129,126,848
logical tensor payload bytes.

The parent verifiers were rerun after extracting their arbitrary-input runtime
engines. Their evidence reports remained byte-identical to FW-0014 and FW-0015,
so the composition refactor did not weaken or alter either earlier gate. The
fixture regenerated byte-identically, and the complete Python and Rust suites
passed (31 Python tests and 23 Rust tests), as did Clippy with warnings denied.

No endpoint tokens were accepted and no TPS was measured. This result is only
the complete attention half of the real layer-0 decoder residual.

## Decision

Pass as a correctness repair. The independent four-stream residual layout,
BF16 multiply/add boundaries, and shared cached DeltaNet state compose exactly.
This promotes the layer-0 MLP hyper-connection and complete decoder-layer
residual to the next correctness slice. No performance default follows from
component parity.
