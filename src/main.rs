use firewing::{
    ...
    benchmark_metal_top10_moe, benchmark_ngram_transport, benchmark_q2_exact_overlap_bound,
...
fn usage() -> ! {
    ...
    eprintln!(
        "  firewing verify-mtp-recursive CHECKPOINT_DIR MODEL_LOCK MTP_SOURCE_LOCK SCHEDULER_LOCK RECURSIVE_LOCK TOKENIZER_FIXTURE NGRAM_FIXTURE NGRAM_ROW_FIXTURE PLE_FIXTURE ENDPOINT_FIXTURE FUSION_FIXTURE CAUSAL_SEED_FIXTURE CAUSAL_ATTENTION_FIXTURE CAUSAL_DECODER_FIXTURE CAUSAL_OUTPUT_FIXTURE RECURSIVE_SEED_FIXTURE RECURSIVE_ATTENTION_FIXTURE RECURSIVE_DECODER_FIXTURE RECURSIVE_OUTPUT_FIXTURE [REPORT_JSON]"
    );
    eprintln!(
        "  firewing verify-mtp-recursive-transaction CHECKPOINT_DIR MODEL_LOCK MTP_SOURCE_LOCK SCHEDULER_LOCK RECURSIVE_LOCK ACCEPTANCE_LOCK TOKENIZER_FIXTURE NGRAM_FIXTURE NGRAM_ROW_FIXTURE PLE_FIXTURE ENDPOINT_FIXTURE FUSION_FIXTURE CAUSAL_SEED_FIXTURE CAUSAL_ATTENTION_FIXTURE CAUSAL_DECODER_FIXTURE CAUSAL_OUTPUT_FIXTURE RECURSIVE_SEED_FIXTURE RECURSIVE_ATTENTION_FIXTURE RECURSIVE_DECODER_FIXTURE RECURSIVE_OUTPUT_FIXTURE TARGET_TRANSACTION_FIXTURE TRANSACTION_FIXTURE [REPORT_JSON]"
    );
    eprintln!(
    ...
    eprintln!(
        "  firewing bench-q2-exact-overlap-bound CHECKPOINT_DIR MODEL_LOCK ROUTER_FIXTURE EXPERT_FIXTURE MIXTURE_FIXTURE ENDPOINT_FIXTURE TRANSACTION_FIXTURE KERNEL_METAL IMPLEMENTATION_COMMIT [REPORT_JSON]"
    );
    eprintln!(
...
fn main() {
    ...
    let (result, output) = match args.get(1).map(String::as_str) {
        ...
        Some("verify-mtp-recursive") if (21..=22).contains(&args.len()) => (
            verify_mtp_recursive_fixture(
                Path::new(&args[2]),
                Path::new(&args[3]),
                Path::new(&args[4]),
                Path::new(&args[5]),
        ...
        Some("verify-mtp-recursive-transaction") if (24..=25).contains(&args.len()) => (
            verify_mtp_recursive_transaction_fixture(
                Path::new(&args[2]),
                Path::new(&args[3]),
                Path::new(&args[4]),
                Path::new(&args[5]),
        ...
        Some("bench-q2-exact-overlap-bound") if (11..=12).contains(&args.len()) => (
            benchmark_q2_exact_overlap_bound(
                Path::new(&args[2]),
                Path::new(&args[3]),
                Path::new(&args[4]),
                Path::new(&args[5]),
